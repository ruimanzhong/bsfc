...
$$
Y_{it} \mid \mu_{it}, \boldsymbol{\theta}_{\mathcal{T}} \stackrel{ind}{\sim} \text{Normal}(\mu_{it},\epsilon^2_{it} ),
$$
...
$$
\epsilon_{it} \mid \boldsymbol{\theta}_{\mathcal{T}} \sim \mathcal{N}(0, 1/\tau_{c_i}^2),
$$
...
In this example, with the number of clusters (`nclust`) set to 15. The output, geodata, contains both the initial graph $\mathbf{G}$ (graph0) and the MST $\mathcal{T}$ ($mstgraph\_ini$), which are essential for subsequent clustering steps.
