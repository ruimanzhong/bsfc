...
bsfc(eta, graphdata = list(graph = graph0, mst = mstgraph.ini, cluster = membership.ini), X = time, N = NULL,
                ...
                correction = T, niter = 1800, burnin = 1000, thin = 1, path_save = path_res) 
 })
```
...
n = length(result[['mst']])
continue_bsfc(eta, graphdata = list(graph = graph0, mst = result[["mst"]][[n]], cluster = result$cluster[n,]), X = time, N = NULL,
                formula, family = "normal", hyperpar = list(c = 0.5),
                correction = T, niter = 3000, burnin = 1000, thin = 1, path_save = path_res) 
```
```{r}
result = readRDS(path_res)
clust_res = result$cluster[nrow(result$cluster),]
p = max(clust_res)
final_model =  lapply(1:p, log_mlik_each, eta, clust_res, X, NULL, formula, correction = F, detailed = T)
...
```{r}
formula= Yk~ f(Xk, model = "rw1", scale.model = T)
log_mlike_vec_res = log_mlik_all(eta,result$cluster[800,], time, N = NULL, formula,family ="normal", correction = T)
log_mlike_res = sum(log_mlike_vec_res)
  log_mlike_vec = log_mlik_all(eta, as.numeric(cluster_true), time, NULL, formula, "normal", correction = T)
