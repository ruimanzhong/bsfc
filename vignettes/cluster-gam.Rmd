...
plotClusterMap(cluster_true = cluster_true,clust_res = as.factor(clust_res), map, file.path(path_im, 'map_gam_poisson_ar1.png'))

plotClusterIter(result$cluster,file.path(path_im, 'gam_prod_poi_ar1.png'))

png(file.path(path_im, 'mlik_poi_ar1.png'), width = 960, height = 480)
  data.frame(id = 1:length(result$log_mlike), log_mlike = result$log_mlike) |>
    ggplot() +
    geom_line(aes(id, log_mlike)) +
    labs(x = "iteration", y = "log marginal likelihood")
...
```{r}
funInterval <- function(k,final_model, n){
  r = final_model[[k]]
r.samples = inla.posterior.sample(100, final_model[[k]])
...
```{r}
png(file.path(path_im, 'fun_poi_gam_ar1.png'), width = 960, height = 480)
