...
```{r}
source('~/Documents/spfc/R/within_clust_model.R')
data = readRDS(file.path(path_sim, 'spatial-data-glm.rds'))
map <-  readRDS(file.path(path_raw, "spatial-domain-clusters.rds"))
...
p = max(cluster_true)
evalLogMLike_each_INLA(2, Y, cluster_true, X = X, N = population, family = "poisson",formula, correction = FALSE, detailed = F)
result_t=lapply( 1:p, FUN = evalLogMLike_each_INLA, Y, cluster_true, X, population, family = "poisson",formula, correction = FALSE, detailed = F)
  log_like_vec_t=unlist(result_t)
  log_like_t=sum(log_like_vec_t)
```

```{r}
path_res <- file.path(path_proj, "data", "result",".RData")
load(file.path(path_res))
clust_res = cluster_out[nrow(cluster_out),]
p = max(clust_res)
result=lapply( 1:p, FUN = evalLogMLike_each_INLA, Y, clust_res, X, population, family = "poisson",formula, correction = FALSE, detailed = F)
  log_like_vec=unlist(result)
