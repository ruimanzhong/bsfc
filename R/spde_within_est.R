evalLogLike_each_INLA <- function(k, Y, X, population, membership, a0 = NULL, b0 = NULL, detailed = F){
  ...
  spde1d <-INLA:: inla.spde2.pcmatern(mesh = mesh1d, alpha = 2,### mesh and smoothness parameter
  ...
  stack <- INLA::inla.stack(data = list(y = data$vec_Yk, e = data$E),
                      A = list(1, A.1d, 1),
                      effects = list(intercept = rep(1, nt*ns),covar.index, idx = 1:length(vec_Yk)),
                      tag = "est")
  
  m.spde <- INLA::inla(formula, data = INLA::inla.stack.data(stack),
